...
export class RealtimeChat {
  ...
  private onMessageCallback?: (messages: ChatMessage[]) => void;
  private onSpeakingCallback?: (speaking: boolean) => void;
  ...
  async connect(): Promise<void> {
    try {
      ...
      // Connect to WebSocket - using project-specific URL
      ...
      this.ws.onmessage = async (event) => {
        ...
        if (data.type === 'response.audio.delta') {
          ...
          if (this.currentTranscript.trim()) {
            this.addMessage(this.currentTranscript.trim(), 'assistant');
  ...
  private startRecording() {
    ...
    this.audioRecorder.start().catch(console.error);
  }
